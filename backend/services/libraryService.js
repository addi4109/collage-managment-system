...
export const editBook = async (bookId, updateData) => {
  const book = await Book.findById(bookId);
  if (!book) {
    throw new Error('Book not found in library catalog.');
  }

  const { title, author, isbn, category, bookCode, totalCopies, finePerDay } = updateData;

  if (bookCode && bookCode !== book.bookCode) {
    const existing = await Book.findOne({ bookCode, _id: { $ne: bookId } });
    if (existing) {
      throw new Error(`Book with code ${bookCode} already exists.`);
    }
  }
  ...
  if (bookCode) book.bookCode = bookCode;
  if (totalCopies !== undefined) book.totalCopies = totalCopies;
  if (finePerDay !== undefined) book.finePerDay = finePerDay;
...
export const deleteBook = async (bookId) => {
  ...
  const activeIssues = await BookIssueLog.countDocuments({
    bookId: book._id,
    status: { $in: ['issued', 'overdue'] },
  });

  if (activeIssues > 0) {
    throw new Error('Cannot delete book with active issues. Return all copies first.');
  }
...
export const renewBook = async (issueLogId, additionalDays = 14) => {
  const log = await BookIssueLog.findById(issueLogId).populate('bookId');
  if (!log) {
    throw new Error('Book issue record not found.');
  }
  ...
  const newDueDate = new Date(log.dueDate);
