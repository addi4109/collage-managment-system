...
export const editBook = async (req, res) => {
  try {
    const book = await libraryService.editBook(req.params.bookId, req.body);
    res.json(book);
  } catch (error) {
    console.error('Edit book error:', error);
    res.status(500).json({ message: error.message || 'Internal server error editing book.' });
  }
};

export const deleteBook = async (req, res) => {
  try {
    const result = await libraryService.deleteBook(req.params.bookId);
    res.json(result);
  } catch (error) {
    console.error('Delete book error:', error);
    res.status(500).json({ message: error.message || 'Internal server error deleting book.' });
  }
};
...
export const renewBook = async (req, res) => {
  try {
    ...
    const log = await libraryService.renewBook(issueLogId, additionalDays || 14);
    res.json(log);
  } catch (error) {
    console.error('Renew book error:', error);
    res.status(500).json({ message: error.message || 'Internal server error renewing book.' });
  }
};
