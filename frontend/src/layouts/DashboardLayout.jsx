...
export default function DashboardLayout({ mode, toggleTheme }) {
  ...
  if (user?.role === 'faculty') {
    ...
    menuItems.push(
      ...
      { text: 'Send Notifications', icon: <NotificationsIcon />, path: `/dashboard/hod?tab=notifications` }
    );
    ...
    menuItems.push(
      ...
      { text: 'Librarian Management', icon: <AutoStoriesIcon />, path: `/dashboard/principal?tab=librarians` },
    ...
    menuItems.push(
      { text: 'Add New Book', icon: <AddIcon />, path: `/dashboard/librarian?tab=add_book` },
      { text: 'Edit Book Catalog', icon: <EditIcon />, path: `/dashboard/librarian?tab=edit_book` },
      { text: 'Delete Books', icon: <DeleteIcon />, path: `/dashboard/librarian?tab=delete_book` },
      { text: 'Issue Book', icon: <BookmarkAddedIcon />, path: `/dashboard/librarian?tab=issue_book` },
      { text: 'Renew Book', icon: <AutorenewIcon />, path: `/dashboard/librarian?tab=renew_book` },
      { text: 'Lending Logs', icon: <HistoryIcon />, path: `/dashboard/librarian?tab=logs` }
    );
  }
            ...
            >
              ...
              <Box sx={{ px: 3, pb: 2, textAlign: 'center' }}>
                ...
                <Chip
                  ...
                  size="small"
                  color={user?.role === 'admin' ? 'secondary' : user?.role === 'faculty' ? 'success' : user?.role === 'librarian' ? 'warning' : 'primary'}
                ...
                <Box sx={{
                  ...
                  {user?.role === 'librarian' && (
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Typography variant="caption" color="text.secondary">Designation</Typography>
                        <Typography variant="caption" sx={{ fontWeight: 'bold' }}>Library Staff</Typography>
                      </Box>
                    </Box>
                  )}
                </Box>
