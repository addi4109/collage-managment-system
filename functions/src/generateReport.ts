...
export const generateReport = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const enrolled: string[] = studentData.enrolledCourses || [];
    let courses: any[] = [];
    ...
    if (submissions.length === 0) {
      ...
      for (const sub of submissions) {
        ...
        const asgDoc = await db.collection('assignments').doc((sub as any).assignmentId).get();
        ...
        doc.fontSize(10).font('Helvetica').fillColor(textGray).text(`Grade: `, { continued: true }).font('Helvetica-Bold').fillColor(primaryColor).text((sub as any).grade || 'Pending Grade');
        
        doc.moveDown(0.5);
        doc.fontSize(9).font('Helvetica-Oblique').fillColor(textGray).text(`Submitted on: ${new Date((sub as any).submittedAt).toLocaleDateString()}`);
        doc.moveDown(0.8);
        doc.moveTo(50, doc.y).lineTo(545, doc.y).strokeColor('#f3f4f6').lineWidth(1).stroke();
        doc.moveDown(0.5);
      }
